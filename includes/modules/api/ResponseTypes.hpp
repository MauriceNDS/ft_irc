...
namespace ResponseTypes {
	...
	const ResponseSpec RPL_WELCOME = ResponseSpec(1, "<nickname> :Welcome to the Internet Relay Network <sendername>");
	const ResponseSpec RPL_YOURHOST = ResponseSpec(2, "<nickname> :Your host is <servername>, running version <ver>");
	const ResponseSpec RPL_CREATED = ResponseSpec(3, "<nickname> :This server was created <date>");
	const ResponseSpec RPL_MYINFO = ResponseSpec(4, "<nickname> <servername> <version> <available user modes> <available channel modes>");
	...
	const ResponseSpec RPL_NAMREPLY = ResponseSpec(353, "<user> = <channel> :<nickname>");
	const ResponseSpec RPL_ENDOFNAMES = ResponseSpec(366, "<user> <channel> :End of NAMES list");
